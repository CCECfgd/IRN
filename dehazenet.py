...
class DehazeNet(nn.Module):
    def __init__(self, input=16, groups=4):
        ...
        self.conv5 = nn.Conv2d(in_channels=48, out_channels=3, kernel_size=5,padding=2)
        # self.brelu = self.BReLU
    ...
    def BRelu(self, x):
        x = torch.max(x, torch.zeros(x.shape[0], x.shape[1], x.shape[2], x.shape[3]).cuda())
        x = torch.min(x, torch.ones(x.shape[0], x.shape[1], x.shape[2], x.shape[3]).cuda())
        return x
